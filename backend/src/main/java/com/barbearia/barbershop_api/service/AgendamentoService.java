...
public class AgendamentoService {
    ...
    public DadosSaidaAgendamento realizarAgendamento(DadosEntradaCadastroAgendamento dto, Usuario usuarioLogado) {
        ...
        if (usuarioLogado.getPerfil() == Perfil.ADMIN) {
            if (dto.clienteId() != null) {
                Optional<Cliente> porId = clienteRepository.findById(dto.clienteId());
                if (porId.isPresent()) {
                    cliente = porId.get();
                } else {
                    Optional<Cliente> porUsuario = clienteRepository.findByUsuarioId(dto.clienteId());
                    if (porUsuario.isPresent()) {
                        cliente = porUsuario.get();
                    }
                }
            }
            if (cliente == null) {
                Optional<Cliente> porUsuarioLogado = clienteRepository.findByUsuarioId(usuarioLogado.getId());
                if (porUsuarioLogado.isPresent()) {
                    cliente = porUsuarioLogado.get();
                } else {
                    ...
                    if (!clientes.isEmpty()) {
                        ...
                        cliente = new Cliente();
                        cliente.setNome("Administrador AfroKings");
                        cliente.setContato("(11) 99999-9999");
                        cliente.setCpf("000.000.000-00");
                        cliente.setEmail(usuarioLogado.getLogin());
            ...
            cliente = clienteRepository.findByUsuarioId(usuarioLogado.getId())
        ...
        // Buscar Barbeiro
        var barbeiro = babeiroRepository.findById(dto.barbeiroId())
                .orElseThrow(() -> new IllegalArgumentException("O profissional selecionado não está mais disponível. Por favor, recarregue e escolha outro barbeiro!"));
        // Buscar serviço
        var servico = servicoRepository.findById(dto.servicoId())
                .orElseThrow(() -> new IllegalArgumentException("Serviço não encontrado com o ID informado!"));
    ...
    private void validarUsuario(Usuario usuarioLogado, Cliente cliente) {
        ...
        boolean ehDono = cliente != null && cliente.getUsuario() != null && usuarioLogado != null && cliente.getUsuario().getId().equals(usuarioLogado.getId());
        if (!ehAdmin && !ehDono) {
